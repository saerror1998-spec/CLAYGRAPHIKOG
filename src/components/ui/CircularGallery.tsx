...
class Media {
  ...
  constructor({
    ...
    img.onerror = () => {
      console.warn('[CircularGallery] Failed to load image:', image);
    };

    this.plane = new Mesh(gl, { geometry, program: this.program });
    this.plane.setParent(scene);

    this.title = new Title({ gl, plane: this.plane, renderer, text, textColor, font });
    this.onResize();
  }
...
class GalleryApp {
  ...
  createMedias(items: { image: string; text: string }[], bend: number, textColor: string, borderRadius: number, font: string) {
    ...
    this.medias = allItems.map((data, index) => {
      return new Media({
  ...
  onTouchDown(e: TouchEvent | MouseEvent) {
    ...
    this.start = e.touches ? e.touches[0].clientX : (e as MouseEvent).clientX;
  }

  onTouchMove(e: TouchEvent | MouseEvent) {
    if (!this.isDown) return;
    const x = e.touches ? e.touches[0].clientX : (e as MouseEvent).clientX;
    this.scroll.target = this.position + (this.start - x) * this.scrollSpeed * 0.025;
  }
  ...
  onWheel(e: WheelEvent) {
    const delta = e.deltaY || e.wheelDelta || (e as unknown as { detail: number }).detail;
...
const Gallery = memo(function CircularGallery({
  items,
  bend = 1,
  textColor = "#F4F4EE",
  borderRadius = 0.03,
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase]);    return (
    <div
      ref={containerRef}
      ...
      style={{ width: "100%", height: "100%", overflow: "hidden", cursor: "grab", position: "relative", zIndex: 10 }}
    />
  );
});
